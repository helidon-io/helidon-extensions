...
/**
 * Stable random samples that do not depend on JDK random-generator implementations.
 */
final class ChaosRandom {
    ...
    static long sample(long streamSeed, long matchedInvocation) {
        return mix64(streamSeed + (matchedInvocation - 1) * GOLDEN_GAMMA);
    }

    static double unitInterval(long sample) {
        return (sample >>> 11) * DOUBLE_UNIT;
    }

    static long boundedLong(long sample, long bound) {
        if (bound <= 0) {
