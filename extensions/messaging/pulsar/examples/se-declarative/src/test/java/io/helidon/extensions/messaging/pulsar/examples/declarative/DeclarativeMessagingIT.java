...
package io.helidon.extensions.messaging.pulsar.examples.declarative;
...
class DeclarativeMessagingIT {
    @Container
    private static final PulsarContainer PULSAR = new PulsarContainer(DockerImageName.parse("apachepulsar/pulsar:4.0.13"));
    ...
    void receivesMessageSentOverHttp() {
        Config config = Config.builder(ConfigSources.create(Map.of(
                        "server.port", "0",
                        "messaging.connector.pulsar-1.service-url", PULSAR.getPulsarBrokerUrl())),
