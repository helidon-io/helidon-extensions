...
# Declarative JMS javax messaging
...
[MessagingEndpoint.java](src/main/java/io/helidon/extensions/messaging/jms/javax/examples/declarative/MessagingEndpoint.java)
...
[ConnectionFactoryProvider.java](src/main/java/io/helidon/extensions/messaging/jms/javax/examples/declarative/ConnectionFactoryProvider.java)
...
`helidon-jms-javax` selects the provider, while `jms-javax-1` names this configured connector
...
This application uses the `jms-javax` connector with the Artemis `artemis-jms-client`
provider. Its POM explicitly supplies `jakarta.jms:jakarta.jms-api:2.0.3`, which
contains the `javax.jms` API. The connector declares that API as `provided`,
so applications must supply it themselves.
...
```shell
mvn clean package
java -jar target/helidon-extensions-messaging-jms-javax-examples-se-declarative.jar
```
