...
import io.helidon.extensions.messaging.connectors.jms.javax.JmsMessage;
import io.helidon.extensions.messaging.tests.jms.javax.JmsMessagingTypes.BytesReceiver;
import io.helidon.extensions.messaging.tests.jms.javax.JmsMessagingTypes.ForwardingReceiver;
import io.helidon.extensions.messaging.tests.jms.javax.JmsMessagingTypes.FtRetryPoisonReceiver;
import io.helidon.extensions.messaging.tests.jms.javax.JmsMessagingTypes.PoisonReceiver;
import io.helidon.extensions.messaging.tests.jms.javax.JmsMessagingTypes.SelectorReceiver;
import io.helidon.extensions.messaging.tests.jms.javax.JmsMessagingTypes.TextReceiver;
import io.helidon.extensions.messaging.tests.jms.javax.JmsMessagingTypes.TextSender;
...
class JmsConnectorIT {
    ...
    void faultToleranceRetryExhaustionIsOneMessagingAttemptBeforeDeadLetter() throws Exception {
        ...
        JmsTestClient.sendText(connectionFactory(), incomingQueue, false, "poison", _ -> {
        });
    ...
    void terminalDeadLetterPublishesPhysicalMessageThenContinues() throws Exception {
        ...
        JmsTestClient.sendText(connectionFactory(), incomingQueue, false, "after poison", _ -> {
        });
    ...
    void terminalDropSettlesPoisonAndContinuesWithNextPhysicalMessage() throws Exception {
        ...
        PoisonReceiver receiver = registry.get(PoisonReceiver.class);
        registry.get(MessagingRuntime.class);
        JmsTestClient.sendText(connectionFactory(), queue, false, "poison", _ -> {
        });
        JmsTestClient.sendText(connectionFactory(), queue, false, "after poison", _ -> {
        });
    ...
    void disabledObjectMessageDropSettlesAndContinues() throws Exception {
        ...
        JmsTestClient.sendObject(connectionFactory(), queue, "untrusted-object", _ -> {
        });
        JmsTestClient.sendText(connectionFactory(), queue, false, "after disabled object", _ -> {
        });
    ...
    void disabledObjectMessageDeadLettersMetadataAndContinues() throws Exception {
        ...
        JmsTestClient.sendText(connectionFactory(), incomingQueue, false, "after disabled object", _ -> {
        });
