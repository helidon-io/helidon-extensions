...
final class ArtemisBroker implements AutoCloseable {
    ...
    static ArtemisBroker create(Path dataDirectory) {
        return new ArtemisBroker(dataDirectory, "helidon-jms-javax-it-" + System.nanoTime());
    }
    ...
    int queueConsumerCount(String queueName) {
        lock.lock();
        try {
    ...
    long queuePendingMessageCount(String queueName) {
        lock.lock();
        try {
    ...
    int queueDeliveringCount(String queueName) {
        lock.lock();
        try {
    ...
    private static boolean hasTemporaryResources(ActiveMQServer server) {
