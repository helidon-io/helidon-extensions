...
import io.helidon.extensions.messaging.connectors.jms.javax.JmsIncomingConfig;
import io.helidon.extensions.messaging.connectors.jms.javax.JmsConnector;
        ...
        try (ArtemisBroker broker = ArtemisBroker.create(temporaryDirectory)) {
            ...
            try (ActiveMQConnectionFactory connectorFactory = new ActiveMQConnectionFactory(broker.connectionUrl())) {
                ...
                try {
                    ...
                    JmsTestClient.sendText(broker.connectionFactory(), queue, false, "first", _ -> { });
                    JmsTestClient.sendText(broker.connectionFactory(), queue, false, "second", _ -> { });
    ...
    void testIncomingConnectionRemainsStoppedUntilGraphActivation() throws Exception {
        ...
        try (ArtemisBroker broker = ArtemisBroker.create(temporaryDirectory.resolve("activation-gate"))) {
            ...
            try (ActiveMQConnectionFactory connectorFactory = new ActiveMQConnectionFactory(broker.connectionUrl())) {
                ...
                JmsTestClient.sendText(broker.connectionFactory(), queue, false, "waiting", _ -> { });
