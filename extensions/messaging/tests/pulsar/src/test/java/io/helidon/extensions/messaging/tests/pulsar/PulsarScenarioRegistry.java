...
final class PulsarScenarioRegistry {
    private static final String FIXTURE_TYPE = PulsarMessagingTypes.class.getName();
    ...
    private static Set<String> scenarioTypes(Class<?>[] fixtureTypes) {
        ...
        for (Class<?> fixtureType : fixtureTypes) {
            ...
            if (!PulsarMessagingTypes.class.equals(actualType.getEnclosingClass())) {
                throw new IllegalArgumentException("Not a Pulsar messaging fixture type: " + actualType.getName());
            }
            result.add(actualType.getSimpleName());
        }
        if (result.isEmpty()) {
            throw new IllegalArgumentException("At least one Pulsar messaging fixture type is required");
        }
