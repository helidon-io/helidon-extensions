...
class JmsOutgoingChannelTest {
    ...
    void perMessageSendFailureReportsPrefixAmbiguousItemAndSuffix() throws Exception {
        ...
        doAnswer(invocation -> {
            ...
            return null;
        }).when(client.producer).send(any(javax.jms.Message.class));
        ...
        verify(client.producer, times(2)).send(any(javax.jms.Message.class));
        connector.close();
    }
    ...
    void jmsRuntimeSendFailureIsAmbiguousAndReconnectsBeforeNextBatch() throws Exception {
        ...
        doThrow(new JMSRuntimeException("provider failed"))
                .when(first.producer)
                .send(any(javax.jms.Message.class));
        AtomicInteger resolutions = new AtomicInteger();
        OutgoingChannel connector = start(config(false), ignored -> resolutions.getAndIncrement() == 0
                ? first.factory
                : second.factory);
    ...
    void transactedSendAndRollbackSameFailureRemainIndeterminate() throws Exception {
        ...
        doThrow(sharedFailure).when(client.producer).send(any(javax.jms.Message.class));
    ...
    void forceCloseAbandonsPerMessageSendThatIgnoresInterruption() throws Exception {
        ...
        try {
            ...
            verify(client.producer, times(1)).send(any(javax.jms.Message.class));
            connector.close();
        } finally {
            releaseSend.countDown();
            connector.forceClose();
