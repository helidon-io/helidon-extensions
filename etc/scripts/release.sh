...
DESCRIPTION: Helidon Extensions Release Script
...
# arg1: pom file
resolve_pom_release_version() {
  local changed latest_version resolved_version projection projection_md5 remote_md5

  # read pom coordinates
  read -r group_id artifact_id version < <(gav "${WS_DIR}/${1}")

  # parse and resolve the latest version
  latest_version=$(resolve_latest_version "${group_id}" "${artifact_id}")
  if [ -n "${latest_version}" ] ; then

    # make a projection of the pom
    projection=$(mktemp)
    cp "${WS_DIR}/${1}" "${projection}"
    set_project_version "${projection}" "${latest_version}"
    ...
    if [ "${projection_md5}" = "${remote_md5}" ] ; then
      ...
      # pom does not need to release
      resolved_version="${latest_version}"
      changed="false"
    else
      # projection does not match latest release
      # pom needs to be released
      resolved_version="${latest_version%%-*}"
      resolved_version="${resolved_version%.*}.$((${resolved_version##*.}+1))"
      changed="true"
    fi
  else
...
update_parents() {
  ...
  for i in "parent/pom.xml" "pom.xml"; do
    ...
    # resolve the version
    read -r version changed < <(resolve_pom_release_version "${i}")
    ...
    while read -r pom; do
      if [ -n "${pom}" ] ; then
        read -r _ parent_artifact_id _ < <(parent_gav "${WS_DIR}/${pom}") || true
...
create_tag() {
  ...
  # Update extension reactor to a single module
  set_module "${WS_DIR}/extensions/pom.xml" "${extension}"
